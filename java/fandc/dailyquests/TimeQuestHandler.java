 ...
 * @author UnAfraid
 */
public class TimeQuestHandler extends AbstractDPScript implements ICommunityBoardHandler, OnPlayerEnterListener {
	...
	@Override
	protected void load() {
		parseDatapackFile("config/mod/TimeQuests.xml");
		log("Loaded TimeQuests data");
	}
	...
	public void onPlayerEnter(Player player) {
		...
		if (!hasQuestTaken) {
			player.sendPacket(new Say2(0, ChatType.TELL, "Mutiny Quest Engine",
					"There are time quests available for you.\nTry them from ALt + B -> Daily Quests"));
		}
	}
				...
				if (st.hasMoreTokens()) {
					...
					for (AbstractDailyQuest quest : QUESTS) {
						if (quest.getName().equals(questName)) {
							...
							onBypassCommand(player, "_bbs_time_quests;info;" + quest.getName() + ";3");
						}
					}
				}
				break;
				...
				for (AbstractDailyQuest quest : QUESTS) {
					if (quest.getName().equals(questName)) {
						...
						if ((qs != null) && (qs.getState() == COMPLETED)
							...
							onBypassCommand(player, "_bbs_time_quests");
		...
		public void sayYes() {
			if (_quest != null) {
				...
				if ((qs != null) && (qs.getState() == STARTED)) {
					...
					onBypassCommand(_player, "_bbs_time_quests");
				}
			}
		}
