...
public class DailyQuestHandler extends AbstractDPScript implements ICommunityBoardHandler, OnPlayerEnterListener {
	private static final AbstractDailyQuest[] QUESTS = new AbstractDailyQuest[] { new GeneralPvPDailyQuest(),
			new ClassSpecificPvPDailyQuest(), new PKHunterDailyQuest(), new FishingDailyQuest(),
			new EnchantingDailyQuest(), new OnlineTimeQuest(), };
...
//          return;
	...
	public void onPlayerEnter(Player player) {
		...
		if (!hasQuestTaken) {
			player.sendPacket(new Say2(0, ChatType.TELL, "Daily Quest Engine",
					"There are daily quests available for you.\nTry them from ALt + B -> Daily Quests"));
		}
	}
...
//      }
	...
	private void sendMainHtml(Player player) {
		...
		for (AbstractDailyQuest quest : QUESTS) {
			...
			if (st == null
					|| (st.getState() == COMPLETED && quest.isRewardClaimed(player.getQuestState(quest.getName()))
			...
			} else if (st.getState() == COMPLETED && !quest.isRewardClaimed(player.getQuestState(quest.getName()))
					&& st.getRestartTime() > System.currentTimeMillis()) {
				sb.append(
