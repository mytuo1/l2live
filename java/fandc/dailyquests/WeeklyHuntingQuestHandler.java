...
import fandc.dailyquests.quests.KillFreyaWeeklyQuest;
import fandc.dailyquests.quests.KillRaidsWeeklyQuest;
import fandc.dailyquests.quests.KillTezzaWeeklyQuest;
import fandc.dailyquests.quests.KillZakenWeeklyQuest;
...
import fandc.dailyquests.quests.WeeklyHuntingBonusQuest;
 ...
 * @author UnAfraid
 */
public class WeeklyHuntingQuestHandler extends AbstractDPScript implements ICommunityBoardHandler, OnPlayerEnterListener {
	private static final AbstractDailyQuest[] QUESTS = new AbstractDailyQuest[] 
		{ 
		  new KillFreyaWeeklyQuest(),
		  new KillZakenWeeklyQuest(),
		  new KillTezzaWeeklyQuest(),
		  new KillRaidsWeeklyQuest(),
		  new WeeklyHuntingBonusQuest(),
		};
	...
	public WeeklyHuntingQuestHandler() {
	...
	@Override
	protected void load() {
		parseDatapackFile("config/mod/WeeklyHuntingQuests.xml");
		log("Loaded Weekly Hunting Quests data");
	}
				...
				if (st.hasMoreTokens()) {
					...
					for (AbstractDailyQuest quest : QUESTS) {
						if (quest.getName().equals(questName)) {
							quest.showInfoWeeklyHunting(player, st);
							break;
						}
					}
				}
				...
				if (st.hasMoreTokens()) {
					...
					for (AbstractDailyQuest quest : QUESTS) {
						if (quest.getName().equals(questName)) {
							...
							onBypassCommand(player, "_bbs_weeklyh_quests;info;" + quest.getName() + ";3");
						}
					}
				}
				break;
				...
				for (AbstractDailyQuest quest : QUESTS) {
					if (quest.getName().equals(questName)) {
						...
						if ((qs != null) && (qs.getState() == COMPLETED)
							...
							onBypassCommand(player, "_bbs_weeklyh_quests");
	...
	private void sendMainHtml(Player player) {
		...
		for (AbstractDailyQuest quest : QUESTS) 
		{
			...
			if ((st == null) || ((st.getState() == COMPLETED) && (st.getRestartTime() <= System.currentTimeMillis())))
			{
				...
				sb.append(
						"<td width=\"200\"><center><button value=\"Info\" action=\"bypass _bbs_weeklyh_quests;info;"
						+ quest.getName()
								+ "\" width=\"110\" height=\"31\" back=\"L2UI_CT2.TestButton.AnimButton0_Down\" fore=\"L2UI_CT2.TestButton.AnimButton0\"></center></td>");
				sb.append("<td width=\"200\"><center><button value=\"Start\" action=\"bypass _bbs_weeklyh_quests;start;"
						+ quest.getName()
						+ "\" width=\"110\" height=\"31\" back=\"L2UI_CT2.TestButton.AnimButton0_Down\" fore=\"L2UI_CT2.TestButton.AnimButton0\"></center></td>");
			}
			...
			else if (!st.isCompleted() && st.isStarted())
			{
				sb.append(
						...
						+ quest.getName()
								+ "\" width=\"110\" height=\"31\" back=\"L2UI_CT2.TestButton.AnimButton0_Down\" fore=\"L2UI_CT2.TestButton.AnimButton0\"></center></td>");
				if 	(quest.getQuestIntId() != 35031)
				{
				sb.append("<td width=\"200\"><center><button value=\"Abort\" action=\"bypass _bbs_weeklyh_quests;abort;"
						+ quest.getName()
						+ "\" width=\"110\" height=\"31\" back=\"L2UI_CT2.TestButton.AnimButton0_Down\" fore=\"L2UI_CT2.TestButton.AnimButton0\"></center></td>");
				}
			}
					...
					&& st.getRestartTime() > System.currentTimeMillis()) 
			{
				sb.append(
						...
						+ quest.getName()
								+ "\" width=\"110\" height=\"31\" back=\"L2UI_CT2.TestButton.AnimButton0_Down\" fore=\"L2UI_CT2.TestButton.AnimButton0\"></center></td>");
				sb.append(
						"<td width=\"200\"><center><button value=\"Claim Reward\" action=\"bypass _bbs_weeklyh_quests;reward;"
								+ quest.getName()
								+ "\" width=\"110\" height=\"31\" back=\"L2UI_CT2.TestButton.AnimButton0_Down\" fore=\"L2UI_CT2.TestButton.AnimButton0\"></center></td>");
			} 
			else 
		...
		public void sayYes() {
			if (_quest != null) {
				...
				if ((qs != null) && (qs.getState() == STARTED)) {
					...
					onBypassCommand(_player, "_bbs_weeklyh_quests");
				}
			}
		}
