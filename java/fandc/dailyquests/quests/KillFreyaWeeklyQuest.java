 ...
 * @author UnAfraid
 */
public class KillFreyaWeeklyQuest extends AbstractDailyQuest
{
	public KillFreyaWeeklyQuest()
	{
	...
	protected String writeQuestInfo(Player player)
	{
		...
		sb.append("Those bosses... especially Freya are setting up an attack on our peaceful world.<br1>");
		sb.append("In order for balance to be restored, we will need you to kill her, make her army afarid.<br1>");
	...
	protected String writeQuestProgress(Player player)
	{
		...
		sb.append("<td fixwidth=60> Freya: </td>");
		sb.append("<td fixwidth=120>" + HtmlUtils.getWeightGauge(450, st.getInt("FREYA_KILLS"), 1, false) + "</td>");
		sb.append("<td fixwidth=5></td>");
		sb.append("</tr>");
		sb.append("</table>");
		return sb.toString();
	...
	{
		st.set("FREYA_KILLS", "0");
		st.set("FREYAS_NEEDED", "0");
	...
	{
		private final AbstractDailyQuest _dq = KillFreyaWeeklyQuest.this;
		@Override
...
//			final Player attackerMember = getRandomPartyMember(attacker, attacker.getQuestState(_dq.getName()));
			final QuestState st = attacker != null ? attacker.getQuestState(getName()) : null;
			if ((attacker == null) || (st == null) || st.isCompleted())
			{
				return;
			}
			if ((actor.getNpcId() == 29180 || actor.getNpcId() == 29179) && st.getInt("FREYA_KILLS") >= st.getInt("FREYAS_NEEDED"))
			{
				st.setState(COMPLETED);
				onQuestFinish(st);
			}
			if ((actor.getNpcId() == 29180 || actor.getNpcId() == 29179) && st.getInt("FREYA_KILLS") == 0)
			{
				st.set("FREYA_KILLS", st.getInt("FREYA_KILLS") + 1);
				showScreenMessage(attacker, "progress " + st.get("FREYA_KILLS") + "/" + " 1"  + " Freya defeated!", 5000);
			}
			else
				return;
