 ...
 * @author UnAfraid
 */
public class KillZakenWeeklyQuest extends AbstractDailyQuest
{
	public KillZakenWeeklyQuest()
	{
	...
	protected String writeQuestInfo(Player player)
	{
		...
		sb.append("Those bosses... especially Zaken are setting up an attack on our peaceful world.<br1>");
		sb.append("In order for balance to be restored, we will need you to kill him, make his army afarid.<br1>");
	...
	protected String writeQuestProgress(Player player)
	{
		...
		sb.append("<td fixwidth=60> Zaken: </td>");
		sb.append("<td fixwidth=120>" + HtmlUtils.getWeightGauge(450, st.getInt("ZAKEN_KILLS"), 1, false) + "</td>");
		sb.append("<td fixwidth=5></td>");
		sb.append("</tr>");
		sb.append("</table>");
		return sb.toString();
	...
	{
		st.set("ZAKEN_KILLS", "0");
		st.set("ZAKENS_NEEDED", "0");
	...
	{
		private final AbstractDailyQuest _dq = KillZakenWeeklyQuest.this;
		@Override
...
//			final Player attackerMember = getRandomPartyMember(attacker, attacker.getQuestState(_dq.getName()));
			...
			if ((actor.getNpcId() == 29022 || actor.getNpcId() == 29176 || actor.getNpcId() == 29181) && st.getInt("ZAKEN_KILLS") >= st.getInt("ZAKENS_NEEDED"))
			{
			...
			if ((actor.getNpcId() == 29022 || actor.getNpcId() == 29176 || actor.getNpcId() == 29181) && st.getInt("ZAKEN_KILLS") <= st.getInt("ZAKENS_NEEDED"))
			{
				st.set("ZAKEN_KILLS", st.getInt("ZAKEN_KILLS") + 1);
				showScreenMessage(attacker, "progress " + st.get("ZAKEN_KILLS") + "/" + " 1"  + " Zaken defeated!", 5000);
			}
			else
				return;
