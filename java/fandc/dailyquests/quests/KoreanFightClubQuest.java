...
public class KoreanFightClubQuest extends AbstractDailyQuest
{
	...
	private class OnKoreanEventExit extends FightClubManager implements OnFCEventStopListener
	{
		...
		public void onEventStop(Player player)
		{
			...
			{
				Log.warn("korean type null or event not NOT_ACTIVE");
			}
			if (event.getType().toString().contentEquals("korean"))
			{
				Log.warn("Found the Event, trying to execute Korean DB tasks" + player.getName() + " .");
				final QuestState st = player.getQuestState(dq.getName());
				...
				{
					showScreenMessage(player, "progress " + st.get("KOREAN_PARTS") + "/" + "10" + " Korean Events completed!", 5000);
				}
			}
		}
	}
