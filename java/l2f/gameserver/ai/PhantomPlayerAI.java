...
//						super.thinkActive();
				...
				*/
				...
				if (actor.getTarget() == null)
				{
					...
					{
						say("fuck it, cant find a target 621");
						castSOE(-1);
					}
				}
			}
			...
			if (actor.getClassId().isMage())
			{
				...
				*/
				...
				for (Skill skill : actor.getAllSkillsArray())
				{
					...
					if (!skill.isActive() || !skill.isMagic() || skill.getCastRange() < 600 || !skill.checkCondition(actor, target, false, false, false))
						continue;
...
//					}
//			if (getActor().getClassId().getLevel() < 4 && // Not 3rd class
//					((getActor().getLevel() >= 76 && getActor().getClassId().getLevel() == 3)))
//					{
//						upgradeClass();		
//					}
			...
			else if (ai.getMood().contains("check warehouse"))
			{
				if (_warehouseChecked < maxChecksWh)
				{
					...
					if (Rnd.chance(10))
						delay = Rnd.get(1, 3) * 60000; // For some reason this guy is afk for 5-15 mins :)
				...
				say("Cant get in the if PPAI 1030");
				rnd = Rnd.get(100);
			}
			else if (ai.getMood().contains("check shop"))
			{
				...
				say("Cant get in the if PPAI 1064");
				if (Rnd.chance(33))
			...
			else if (ai.getMood().contains("check npcs"))
			{
				...
				say("Cant get in the if PPAI 1095");
	...
	public void startRoamingInTown()
	{
		...
		_roamingTask = ThreadPoolManager.getInstance().schedule(new RoamingTask(getActor().getPlayer()), 30000); //lets wait 30sec before start move or etc..
	}
	...
	public void startFarming()
	{
		...
		_farmingTask = ThreadPoolManager.getInstance().schedule(new FarmingTask(getActor().getPlayer()), 5000);
	}
		...
		public void run()
		{
			...
			try
			{
				...
				if (ai.isInTown())
				{
					...
					{
						if (npc != null && npc.getTitle().equalsIgnoreCase("Global Gatekeeper"))
						{
			...
			finally
			{
				if (_isInterrupted)
				{
					Log.warn("interrupted farm task");
					ai.setMood("");
				}
			}
		}
...
//			checkEquipment();
	...
	{
		equipWeapon(PhantomPlayers.getAppropreateWeaponType(getActor().getPlayer()));
	}

	private void checkEquipment()
	{
		PhantomPlayers.equipArmor(getActor().getPlayer(), getActor().getPlayer().getGrade());
		PhantomPlayers.equipJewels(getActor().getPlayer(), getActor().getPlayer().getGrade());
		PhantomPlayers.equipWeapon(getActor().getPlayer(), getActor().getPlayer().getGrade());
	}
...
//					actor.getInventory().addItem(soulId, 1000 - shotsCount, "Phantom");
				Functions.addItem(getActor().getPlayer(), soulId, 10000, "Phantom SS");
			}
...
//					actor.getInventory().addItem(bspiritId, 10000 - shotsCount, "Phantom");
				Functions.addItem(getActor().getPlayer(), bspiritId, 10000, "Phantom BSS");
			}
	...
	public void onKill(Creature actor, Creature victim)
	{
		...
		//adding support for class change on kill
		if (!victim.isPlayer())
		{
			if (getActor().getPlayer().getExp() >= Experience.getExpForLevel(20) && getActor().getPlayer().getExp() < Experience.getExpForLevel(40))
			{
				upgradeClass();
			}
			else if (getActor().getPlayer().getExp() >= Experience.getExpForLevel(40) && getActor().getPlayer().getExp() < Experience.getExpForLevel(76))
			{
				upgradeClass();
			}
			else if (getActor().getPlayer().getExp() >= Experience.getExpForLevel(76))
			{
				upgradeClass();
			}
		}
