...
public class FortInformerInstance extends NpcInstance
{
	Fortress fortress;// = ResidenceHolder.getInstance().getResidence(Fortress.class, Integer.parseInt(command.substring(5)));
	FortressSiegeEvent siegeEvent;
	
	public FortInformerInstance(int objectId, NpcTemplate template)
	{
...
//		FortressSiegeEvent siegeEvent = fortress.getSiegeEvent();
		...
		if (command.startsWith("fort_"))
		{
			NpcHtmlMessage html = new NpcHtmlMessage(player, this);
			Fortress _fortress = ResidenceHolder.getInstance().getResidence(Fortress.class, Integer.parseInt(command.substring(5)));
			FortressSiegeEvent _siegeEvent = _fortress.getSiegeEvent();
			if (_fortress.getOwner() != null)
			{
				html.setFile("residence2/fortress/fortress_ordery001a.htm");
				html.replace("%clan_name%", _fortress.getOwner().getName());
			}
			else
				html.setFile("residence2/fortress/fortress_ordery013.htm");
				
				player.sendPacket(html);
				fortress = _fortress;
				siegeEvent = _siegeEvent;
