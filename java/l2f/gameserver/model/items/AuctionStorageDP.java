...
public class AuctionStorageDP extends ItemContainer
{
	private static final Logger _log = LoggerFactory.getLogger(AuctionStorageDP.class);
	private static final ItemsDAO _itemsDAO = ItemsDAO.getInstance();
	private static AuctionStorageDP _instance;

	protected AuctionStorageDP()
	{
		restore();
	}
	...
	public void restore()
	{
		...
		{
			_items.clear();
			Collection<ItemInstance> items = _itemsDAO.getItemsByLocation(ItemLocation.AUCTIONDP);
	...
	public static AuctionStorageDP getInstance()
	{
		if (_instance == null)
			_instance = new AuctionStorageDP();
		return _instance;
	}
}