...
public class AuctionManagerFA
{
	private static AuctionManagerFA _instance;
	private static final Logger _log = LoggerFactory.getLogger(AuctionManagerFA.class);
	...
	private void loadAuctions()
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("SELECT * FROM auctionsfa");
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("INSERT INTO auctionsfa VALUES(?,?,?,?,?)"))
		{
	 ...
	 * @param adena 
	 */
	public void addFAToSeller(int sellerObjectId, long fa)
	{
		int objId = -1;

		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT object_id FROM items WHERE item_id=6673 AND owner_id="+sellerObjectId+" AND loc='INVENTORY'");
		     ResultSet rset = statement.executeQuery())
		{
			if (rset.next())
		...
		{
			_log.error("Error while selecting FA:", e);
		}

		if (objId == -1)
		{
			ItemInstance item = ItemFunctions.createItem(6673);
			item.setCount(fa);
		...
		{
			try (Connection con = DatabaseFactory.getInstance().getConnection();
			     PreparedStatement statement = con.prepareStatement("UPDATE items SET count=count+"+fa+" WHERE object_id="+objId))
			{
				statement.execute();
			}
			catch (SQLException e)
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("DELETE FROM auctionsfa WHERE auction_id = ?"))
		{
			statement.setInt(1, auction.getAuctionId());
			statement.execute();
		}
	...
	public void buyItem(Player buyer, ItemInstance item, long quantity)
	{
		...
		if (auction.getPricePerItem()*quantity > buyer.getFA())
		{
			sendMessage(buyer, "You don't have enough FA!");
			return;
		}
		...
		buyer.getInventory().reduceFA(auction.getPricePerItem()*quantity, "Auction Online Bought");
		...
		if (seller != null)
		{
			...
			seller.addFA(auction.getPricePerItem()*quantity, true, "Auction Online Sold");
		}
		else
		{
			addFAToSeller(auction.getSellerObjectId(), auction.getPricePerItem()*quantity);
			Log.LogAddItem(auction.getSellerName() + '[' + auction.getSellerObjectId() + ']', "Auction Offline", "FA", auction.getPricePerItem() * quantity);
		}
	...
	public void checkAndAddNewAuction(Player seller, ItemInstance item, long quantity, long salePrice)
	{
		...
		seller.getInventory().reduceFA(Config.AUCTION_FEE2, "Create Auctino Fee");
	...
	private boolean checkIfItsOk(Player seller, ItemInstance item, long quantity, long salePrice)
	{
		...
		if (seller.getFA() < Config.AUCTION_FEE2)
		{
			sendMessage(seller, "You don't have enough FA, to pay the fee!");
			return false;
		}
		if (salePrice <= 0)
		{
	...
	public static AuctionManagerFA getInstance()
	{
		if (_instance == null)
			_instance = new AuctionManagerFA();
		return _instance;
	}
}
