...
public class AuctionManagerDP
{
	private static AuctionManagerDP _instance;
	private static final Logger _log = LoggerFactory.getLogger(AuctionManagerDP.class);
	...
	private void loadAuctions()
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("SELECT * FROM auctionsdp");
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("INSERT INTO auctionsdp VALUES(?,?,?,?,?)"))
		{
	 ...
	 * @param adena 
	 */
	public void addDPToSeller(int sellerObjectId, long dp)
	{
		int objId = -1;

		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT object_id FROM items WHERE item_id=37000 AND owner_id="+sellerObjectId+" AND loc='INVENTORY'");
		     ResultSet rset = statement.executeQuery())
		{
			if (rset.next())
		...
		{
			_log.error("Error while selecting DP:", e);
		}

		if (objId == -1)
		{
			ItemInstance item = ItemFunctions.createItem(37000);
			item.setCount(dp);
		...
		{
			try (Connection con = DatabaseFactory.getInstance().getConnection();
			     PreparedStatement statement = con.prepareStatement("UPDATE items SET count=count+"+dp+" WHERE object_id="+objId))
			{
				statement.execute();
			}
			catch (SQLException e)
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("DELETE FROM auctionsdp WHERE auction_id = ?"))
		{
			statement.setInt(1, auction.getAuctionId());
			statement.execute();
		}
	...
	{
		if (!Config.AUCTION_PRIVATE_STORE_AUTO_ADDEDDP)
			return;
	...
	public void buyItem(Player buyer, ItemInstance item, long quantity)
	{
		...
		if (auction.getPricePerItem()*quantity > buyer.getDP())
		{
			sendMessage(buyer, "You don't have enough DP!");
			return;
		}
		...
		buyer.getInventory().reduceDP(auction.getPricePerItem()*quantity, "Auction Online Bought");
		...
		if (seller != null)
		{
			...
			seller.addDP(auction.getPricePerItem()*quantity, true, "Auction Online Sold");
		}
		else
		{
			addDPToSeller(auction.getSellerObjectId(), auction.getPricePerItem()*quantity);
			Log.LogAddItem(auction.getSellerName() + '[' + auction.getSellerObjectId() + ']', "Auction Offline", "DP", auction.getPricePerItem() * quantity);
		}
	...
	public void checkAndAddNewAuction(Player seller, ItemInstance item, long quantity, long salePrice)
	{
		...
		seller.getInventory().reduceDP(Config.AUCTION_FEE2, "Create Auctino Fee");
	...
	private boolean checkIfItsOk(Player seller, ItemInstance item, long quantity, long salePrice)
	{
		...
		if (seller.getDP() < Config.AUCTION_FEE2)
		{
			sendMessage(seller, "You don't have enough DP, to pay the fee!");
			return false;
		}
		if (salePrice <= 0)
		{
	...
	public static AuctionManagerDP getInstance()
	{
		if (_instance == null)
			_instance = new AuctionManagerDP();
		return _instance;
	}
}
