...
public class CCPSecondaryPassword
{
		...
		switch (pageIndex)
		{
			case "C"://Change Password
				...
				if (getSecondaryPass(player).equals(currentPass))
				{
					...
					if(!isValid(mail))
					{
						player.sendMessage("You have to use a real e-mail!");
						return;
					}
					setSecondaryPassword(player, player.getAccountName(), newPass, mail);
				}
				else
				{
			...
			case "S":// Setup Password
				...
				setSecondaryPassword(player, player.getAccountName(), args[1], args[2]);
				break;
		}
	...
	public static void setSecondaryPassword(Player player, String accountName, String password, String mail)
	{
		...
		try (Connection con = LoginDatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("UPDATE accounts SET secondaryPassword=?, l2email=? WHERE login=?"))
		{
