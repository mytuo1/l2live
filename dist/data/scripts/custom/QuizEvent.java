...
public class QuizEvent
{
	...
	private static int _initWait = 4200;

	// Time for answer the question (in seconds)
	private static int _answerTime = 15;

	// Time between two event (in seconds) 7200
	private static int _betweenTime = 21,600‬;
	...
	private static void announceStart()
	{
		...
		Announcements.getInstance().announceToAll("Get Ready! L2Mutiny is ready to reward you!");
	...
	{
		_quizRunning = false;
		Announcements.getInstance().announceToAll("The Quiz Event is over! Thank you for participating!");
		announced = 0;
		_status = STATUS_NOT_IN_PROGRESS;
	...
	private static void giveReward()
	{
		for (Player p : _players.keySet())
		{
			...
			{
				p.sendMessage("Your answer was correct! L2Mutiny Reward you with 1 GCM!");
