...
{
	private static final Logger _log = LoggerFactory.getLogger(CommunityAuctionHouseDP.class);
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		if ("maillistDP".equals(cmd))
		{

			html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_listdp.htm", player);
				...
				if (st.hasMoreTokens())
				{
					...
					if (action == 1)
					{
						...
						else
						{
							...
							Auction auction = AuctionManagerDP.getInstance().getAuction(auctionId);
					...
					html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_buy_itemdp.htm", player);
		...
		else if (cmd.equals("newAuctionDP"))
		{
			...
			if (buttonClicked != null)
			{
				...
				else if (buttonClicked.equals("1"))
				{
					...
					{
						player.sendMessage("Item has been already sold");
						Collection<Auction> auctions = AuctionManagerDP.getInstance().getMyAuctions(player.getObjectId());
						for (Auction a : auctions)
						{
							...
							AuctionManagerDP.getInstance().removeStore(player, a.getAuctionId());
						}
					}
					else
					{
			...
			html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_new_auctiondp.htm", player);
			...
			String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort)
	{
			...
			if (itemTypes[0] == i)
			{
				...
				for (AuctionItemTypes itemType : types)
				{
					...
					builder.append("<button value=\"").append(itemType.toString().replace("_", " "))
							.append("\" action=\"bypass _auctionDP_ 1 _ ").append(itemTypes[0]).append(" ")
							.append(count)
		...
		StringBuilder builder = new StringBuilder();
		Collection<Auction> allAuctions = AuctionManagerDP.getInstance().getAllAuctions();
		...
		for (int i = 10*(page-1);i<Math.min(auctions.size(), 10*(page));i++)
		{
			...
			builder.append("DC " + "<font color=FF6633 name=CREDITTEXTSMALL>("+ Util.getNumberWithCommas(auction.getPricePerItem()) + ")</font>" + "<br1>");
		...
		html = html.replace("%adena%", 			Util.getNumberWithCommas(player.getDP()));
		return html;
	}
			...
			String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort, int auctionId)
	{
		...
		html = html.replace("%totalAdena%", 	"<font color=94775b>"+String.valueOf(Util.getNumberWithCommas(player.getDP()))+"</font>");
	...
	private String fillNewAuctionPage(Player player, String html, boolean newItem, int currentItem, int line)
	{
		...
		Collection<Auction> myAuctions = AuctionManagerDP.getInstance().getMyAuctions(player);
	...
	private List<ItemInstance> getItemsToAuction(Player player)
	{
		...
		for (ItemInstance item : inventory.getItems())
		{
			...
			if (item.isAugmented())
				continue;
			if (item.isStackable())
				for (Auction playerAuction : AuctionManagerDP.getInstance().getMyAuctions(player))
