...
public class RankingCommunity implements ScriptFile, ICommunityBoardHandler
{
	...
	private final int TIME_UPDATE = 10 * 60 * 1000;
	...
	private static enum RankingEnum
	{
		...
		ENCHANT("Enchants Succeeded", "ench", "ench_s", RankingType.NORMAL),
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		switch (type)
		{
			...
			case "oly":
				...
				break;
			case "ench":
				showRanking(player, RankingEnum.ENCHANT, page);
				break;
			case "adena":
//				if (player.getAccessLevel() > 0)
	...
	private void showRanking(Player player, RankingEnum type, int page)
	{
		...
		html = html.replace("%adenaBypass%", "<td align=center><button action=\"bypass _bbsranking:adena\" value=\"Adena\" width=120 height=25 back=\"cb.mx_button_down\" fore=\"cb.mx_button\"></td>");
...
//		player.sendMessage("Page: " + page);
		...
		for (int index = page * scoresPerPage; index < ((page * scoresPerPage) + scoresPerPage); index++)
		{
			if (stats.size() > index)
			{
				final RankingStats playerStats = stats.get(index);
				sb.append("<table width=540 height=25 " + (playerStats.getPlayerId() == player.getObjectId() ? "bgcolor=3a0000" : (index % 2 == 0 ? "bgcolor=1F1C19" : "")) + ">");
				sb.append("<tr>");
				sb.append("<td align=center>");
				sb.append("<table>");
				sb.append("<tr>");
				sb.append("<td width=25 valign=top>");
				sb.append("<font name=CreditTextNormal color=D2B48C>" + (index + 1) + "</font>");
				sb.append("</td>");
			...
			{
				sb.append("<table width=540 height=25 " + (index % 2 == 0 ? "" : "bgcolor=1F1C19") + ">");
				sb.append("<tr>");
				sb.append("<td align=center>");
				sb.append("<table>");
				sb.append("<tr>");
		...
		if (page > 0)
		{
			if (type.getBypass() == "pvp")
			{
				previousPage = "<br1><button action=\"bypass _bbsranking:" + "pk" + ":" + (page - 1) + "\" width=15 height=15 back=\"L2UI_CH3.prev1_down\" fore=\"L2UI_CH3.prev1\">";
			}
			else if (type.getBypass() == "pk")
			{
				previousPage = "<br1><button action=\"bypass _bbsranking:" + "pvp" + ":" + (page - 1) + "\" width=15 height=15 back=\"L2UI_CH3.prev1_down\" fore=\"L2UI_CH3.prev1\">";
			}
			else
		...
		{
			if (type.getBypass() == "pvp") //  this is a temporary fix, until we come back to it later. otherwise it messes the pages up
			{
				nextPage = "<br1><button action=\"bypass _bbsranking:" + "pk" + ":" + (page + 1) + "\" width=15 height=15 back=\"L2UI_CH3.next1_down\" fore=\"L2UI_CH3.next1\">";
			}
			else if (type.getBypass() == "pk")
			{
				nextPage = "<br1><button action=\"bypass _bbsranking:" + "pvp" + ":" + (page + 1) + "\" width=15 height=15 back=\"L2UI_CH3.next1_down\" fore=\"L2UI_CH3.next1\">";
			}
			else
		...
		html = html.replace("<?update?>", String.valueOf(TIME_UPDATE / (60 * 1000)));
		html = html.replace("<?last_update?>", TIME_FORMAT.format(_lastUpdate));
		html = html.replace("%adenaBypass%", "<tr><td height=40 align=center><button action=\"bypass _bbsranking:adena\" value=\" Adena\" width=110 height=31 back=\"L2UI_CT2.TestButton.AnimButton0_Down\" fore=\"L2UI_CT2.TestButton.AnimButton0\"></td></tr>");
		ShowBoard.separateAndSend(html, player);
	}
