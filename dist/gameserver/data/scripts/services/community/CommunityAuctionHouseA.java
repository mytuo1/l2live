...
{
	private static final Logger _log = LoggerFactory.getLogger(CommunityAuctionHouseA.class);
	...
	{
		return new String[] { "_maillistA", "_bbsAuctiona_", "_bbsNewAuctiona"};
	}
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		if ("maillistA".equals(cmd))
		{

			html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_lista.htm", player);

			html = fillAuctionListPage(player, html, 1, new int[] {-1, -1}, "All", null, 1, 0, 0, 0);

		}
		//Auction List page
		else if (cmd.equals("bbsAuctiona"))
		{
			try
			{
				//page
				...
				if (st.hasMoreTokens())
				{
					...
					if (action == 1)
					{
						...
						else
						{
							...
							Auction auction = AuctionManagerA.getInstance().getAuction(auctionId);
					...
					html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_buy_itema.htm", player);
		...
		else if (cmd.equals("bbsNewAuctiona"))
		{
			...
			if (buttonClicked != null)
			{
				...
				else if (buttonClicked.equals("1"))
				{
					...
					{
						player.sendMessage("Item has been already sold");
						Collection<Auction> auctions = AuctionManagerA.getInstance().getMyAuctions(player.getObjectId());
						for (Auction a : auctions)
						{
							...
							AuctionManagerA.getInstance().removeStore(player, a.getAuctionId());
						}
					}
					else
					{
			...
			html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_new_auctiona.htm", player);
			...
			String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort)
	{
		...
		for (int i = 1;i<=6;i++)
		{
			if (itemTypes[0] == i)
			{
				...
				html = html.replace("%plusMinusBtn"+i+"%", "<button value=\"\" action=\"bypass _bbsAuctiona_ 1 _ -1 -1 _ %grade% _ %search% _ %itemSort% _ %gradeSort% _ %quantitySort% _ %priceSort%\" width=15 height=15 back=\"L2UI_CH3.QuestWndMinusBtn\" fore=\"L2UI_CH3.QuestWndMinusBtn\">");
			...
			{
				html = html.replace("%plusMinusBtn"+i+"%", "<button value=\"\" action=\"bypass _bbsAuctiona_ 1 _ "+(i)+" -1 _ %grade% _ %search% _ %itemSort% _ %gradeSort% _ %quantitySort% _ %priceSort%\" width=15 height=15 back=\"L2UI_CH3.QuestWndPlusBtn\" fore=\"L2UI_CH3.QuestWndPlusBtn\">");
		...
		StringBuilder builder = new StringBuilder();
		Collection<Auction> allAuctions = AuctionManagerA.getInstance().getAllAuctions();
		...
		for (int i = 10*(page-1);i<Math.min(auctions.size(), 10*(page));i++)
		{
			...
			builder.append("Adena " + "<font color=FF6633 name=CREDITTEXTSMALL>("+ Util.getNumberWithCommas(auction.getPricePerItem()) + ")</font>" + "<br1>");
	...
	private String fillNewAuctionPage(Player player, String html, boolean newItem, int currentItem, int line)
	{
		...
		for (int i = 6 * (line); i < 6 * (line + 3); i++)
		{
			...
			if (item != null)
				builder.append("<button value=\"\" action=\"bypass _bbsNewAuctiona_ n").append(item.getObjectId()).append(" _ ").append(line).append("\" width=32 height=32 back=L2UI_CT1.ItemWindow_DF_Frame_Down fore=L2UI_CT1.ItemWindow_DF_Frame />");
			else
				builder.append("<br>");
			builder.append("</td>");
			builder.append("</tr>");
		...
		Collection<Auction> myAuctions = AuctionManagerA.getInstance().getMyAuctions(player);
		...
		for (;i<10;i++)
		{
			...
			if (!player.hasDialogAskActive())
				builder.append("<button value=\"\" action=\"bypass _bbsNewAuctiona_ c").append(item.getObjectId()).append(" _ ").append(line).append(" _ 1\" width=32 height=32 back=\"L2UI_CT1.ItemWindow_DF_Frame_Down\" fore=\"L2UI_CT1.ItemWindow_DF_Frame\">");
			else
	...
	private List<ItemInstance> getItemsToAuction(Player player)
	{
		...
		for (ItemInstance item : inventory.getItems())
		{
			...
			if (item.isAugmented())
				continue;
			if (item.isStackable())
				for (Auction playerAuction : AuctionManagerA.getInstance().getMyAuctions(player))
		...
		public void sayYes()
		{
			switch (_button)
			{
				case New_Auction:
					...
					{
						onBypassCommand(_player, "_bbsNewAuctiona_ c0 _ 0");
						return;
					}
					AuctionManagerA.getInstance().checkAndAddNewAuction(_player, _item, quantity, pricePerItem);
					onBypassCommand(_player, "_bbsNewAuctiona_ c0 _ 0");
					break;
				case Cancel_Auction:
					AuctionManagerA.getInstance().deleteAuction(_player, _item);
					onBypassCommand(_player, "_bbsNewAuctiona_ c0 _ 0");
					break;
				case Buy_Item:
					AuctionManagerA.getInstance().buyItem(_player, _item, Long.parseLong(_args[0]));
					onBypassCommand(_player, "_maillistA");
					break;
			}
		}
