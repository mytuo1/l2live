...
public class CommunityAuctionHouseFA implements ScriptFile, ICommunityBoardHandler
{
	private static final Logger _log = LoggerFactory.getLogger(CommunityAuctionHouseFA.class);
	...
	public void onLoad()
	{
		...
		{
			_log.info("CommunityBoard: Auction System Service FA loaded.");
	...
	{
		return new String[] { "_maillistFA", "_auctionFA_", "_newAuctionFA"};
	}
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		if ("maillistFA".equals(cmd))
		{

			html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_listfa.htm", player);

			html = fillAuctionListPage(player, html, 1, new int[] {-1, -1}, "All", null, 1, 0, 0, 0);

		}
		//Auction List page
		else if (cmd.equals("auctionFA"))
		{
			try
			{
				//page
				...
				if (st.hasMoreTokens())
				{
					...
					if (action == 1)
					{
						...
						else
						{
							...
							Auction auction = AuctionManagerFA.getInstance().getAuction(auctionId);
							...
							else
							{
								...
								ConfirmDlg packet = new ConfirmDlg(SystemMsg.S1, 60000)
										.addString("Are you sure, you want to buy " + quantity + ' ' + item.getName()
												+ " for " + Util.getNumberWithCommas(realPrice) + " Festival Adena?");
								player.ask(packet, new ButtonClick(player, item, Buttons.Buy_Item, quantity));
							}
						}
					}
					//sending buy_item html
					html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_buy_itemfa.htm", player);
		...
		else if (cmd.equals("newAuctionFA"))
		{
			...
			if (buttonClicked != null)
			{
				...
				else if (buttonClicked.equals("1"))
				{
					...
					{
						player.sendMessage("Item has been already sold");
						Collection<Auction> auctions = AuctionManagerFA.getInstance().getMyAuctions(player.getObjectId());
						for (Auction a : auctions)
						{
							...
							AuctionManagerFA.getInstance().removeStore(player, a.getAuctionId());
						}
					}
					else
					{
			...
			html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_mail_new_auctionfa.htm", player);
			...
			String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort)
	{
		...
		for (int i = 1;i<=6;i++)
		{
			if (itemTypes[0] == i)
			{
				...
				html = html.replace("%plusMinusBtn"+i+"%", "<button value=\"\" action=\"bypass _auctionFA_ 1 _ -1 -1 _ %grade% _ %search% _ %itemSort% _ %gradeSort% _ %quantitySort% _ %priceSort%\" width=15 height=15 back=\"L2UI_CH3.QuestWndMinusBtn\" fore=\"L2UI_CH3.QuestWndMinusBtn\">");
			...
			{
				html = html.replace("%plusMinusBtn"+i+"%", "<button value=\"\" action=\"bypass _auctionFA_ 1 _ "+(i)+" -1 _ %grade% _ %search% _ %itemSort% _ %gradeSort% _ %quantitySort% _ %priceSort%\" width=15 height=15 back=\"L2UI_CH3.QuestWndPlusBtn\" fore=\"L2UI_CH3.QuestWndPlusBtn\">");
		...
		StringBuilder builder = new StringBuilder();
		Collection<Auction> allAuctions = AuctionManagerFA.getInstance().getAllAuctions();
		...
		html = html.replace("%adena%", 			Util.getNumberWithCommas(player.getFA()));
		return html;
	}
			...
			String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort, int auctionId)
	{
		...
		html = html.replace("%totalAdena%", 	"<font color=94775b>"+String.valueOf(Util.getNumberWithCommas(player.getFA()))+"</font>");
	...
	private String fillNewAuctionPage(Player player, String html, boolean newItem, int currentItem, int line)
	{
		...
		for (int i = 6 * (line); i < 6 * (line + 3); i++)
		{
			...
			if (item != null)
				builder.append("<button value=\"\" action=\"bypass _newAuctionFA_ n").append(item.getObjectId()).append(" _ ").append(line).append("\" width=32 height=32 back=L2UI_CT1.ItemWindow_DF_Frame_Down fore=L2UI_CT1.ItemWindow_DF_Frame />");
			else
				builder.append("<br>");
			builder.append("</td>");
			builder.append("</tr>");
		...
		Collection<Auction> myAuctions = AuctionManagerFA.getInstance().getMyAuctions(player);
		...
		for (;i<10;i++)
		{
			...
			if (!player.hasDialogAskActive())
				builder.append("<button value=\"\" action=\"bypass _newAuctionFA_ c").append(item.getObjectId()).append(" _ ").append(line).append(" _ 1\" width=32 height=32 back=\"L2UI_CT1.ItemWindow_DF_Frame_Down\" fore=\"L2UI_CT1.ItemWindow_DF_Frame\">");
			else
		...
		html = html.replace("%saleFee%", Util.getNumberWithCommas(Config.AUCTION_FEE2));
		html = html.replace("%adena%", Util.getNumberWithCommas(player.getFA()));
		...
		html = html.replace("%NewAuctionButton%", (choosenItem != null ? "<center><button value=\"New Auction\" action=\"bypass _newAuctionFA_ "+(newItem ? "n" : "c")+currentItem+" _ "+line+" _ 0 _ "+(choosenItem == null || choosenItem.getCount() > 1 ? "$quantity" : "1" )+" _ $sale_price\" width=90 height=30 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></center>" : ""));
	...
	private List<ItemInstance> getItemsToAuction(Player player)
	{
		...
		for (ItemInstance item : inventory.getItems())
		{
			...
			if (item.isAugmented())
				continue;
			if (item.isStackable())
				for (Auction playerAuction : AuctionManagerFA.getInstance().getMyAuctions(player))
		...
		public void sayYes()
		{
			switch (_button)
			{
				case New_Auction:
					...
					{
						onBypassCommand(_player, "_newAuctionFA_ c0 _ 0");
						return;
					}
					AuctionManagerFA.getInstance().checkAndAddNewAuction(_player, _item, quantity, pricePerItem);
					onBypassCommand(_player, "_newAuctionFA_ c0 _ 0");
					break;
				case Cancel_Auction:
					AuctionManagerFA.getInstance().deleteAuction(_player, _item);
					onBypassCommand(_player, "_newAuctionFA_ c0 _ 0");
					break;
				case Buy_Item:
					AuctionManagerFA.getInstance().buyItem(_player, _item, Long.parseLong(_args[0]));
					onBypassCommand(_player, "_maillistFA");
					break;
			}
		}
