...
//				html = html.replace("{feet_grade}", feet.getItemGrade().name());
		...
		else if (command.equals("dress-armor"))
		{
			...
			if (chest.getTemplate().getCrystalType() == ItemTemplate.Grade.NONE || chest.getTemplate().getCrystalType() == ItemTemplate.Grade.D || chest.getTemplate().getCrystalType() == ItemTemplate.Grade.C || chest.getTemplate().getCrystalType() == ItemTemplate.Grade.B)
			{
				player.sendMessage("Error: You can not set Visual on Armors below A grade");
				useVoicedCommand("dress-armorpage", player, args);
				return false;
			}
			...
			if (chest.getTemplate().getBodyPart() == ItemTemplate.SLOT_FULL_ARMOR && chest.getTemplate().getBodyPart() != visual.getBodyPart())
			{
				player.sendMessage("Error: You can't change a full body armor set to 5 piece set.");
				useVoicedCommand("dress-armorpage", player, args);
				return false;
			}
			...
			ArmorSet armorSet = ArmorSetsHolder.getInstance().getArmorSet(chest.getItemId());
//			if (armorSet == null) {
//				player.sendMessage("Error: You can't change Visual to Items without a Set!");
//				useVoicedCommand("dress-armorpage", player, args);
//				return false;
//			}
		...
		else if (command.equals("dress-weapon"))
		{
			...
			if (weapon.getTemplate().getCrystalType() == ItemTemplate.Grade.NONE || weapon.getTemplate().getCrystalType() == ItemTemplate.Grade.D || weapon.getTemplate().getCrystalType() == ItemTemplate.Grade.C || weapon.getTemplate().getCrystalType() == ItemTemplate.Grade.B || weapon.getTemplate().getCrystalType() == ItemTemplate.Grade.A)
			{
				player.sendMessage("Error: You can not set Visual on Weapons below S grade");
				useVoicedCommand("dress-armorpage", player, args);
				return false;
			}
